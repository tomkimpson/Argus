...
- Auto-detection of results files in current directory
...
Usage: python create_corner_plot.py <num_pulsars> [smooth_sigma] [--plot_log10_gamma_a] [--plot_priors] [--efac] [--equad] [--plot_injections]

Examples:
  python create_corner_plot.py 2                                # Basic plot
  python create_corner_plot.py 2 1.0                            # With smoothing
  python create_corner_plot.py 2 0.5 --plot_log10_gamma_a      # Include log10_gamma_a
  python create_corner_plot.py 2 1.0 --plot_priors              # With prior overlays
  python create_corner_plot.py 2 --efac --equad                 # Include EFAC and EQUAD
  python create_corner_plot.py 2 --efac --equad --plot_priors   # EFAC/EQUAD with priors
  python create_corner_plot.py 2 --efac --equad --plot_injections # EFAC/EQUAD with injections
  python create_corner_plot.py 2 1.0 --plot_log10_gamma_a --plot_priors # Full featured
...
Features:
- Automatically detects results files (*_results.nc) in current directory tree
...
def load_config(run_identifier):
    """Load configuration file for the given run identifier."""
    # Try multiple config file patterns
    potential_config_files = [
        f"../argus/configs/config_numpyro_test_{run_identifier}.ini",
        f"configs/config_numpyro_test_{run_identifier}.ini", 
        f"../configs/config_numpyro_test_{run_identifier}.ini",
    ]
    
    config_file = None
    for potential_file in potential_config_files:
        if os.path.exists(potential_file):
            config_file = potential_file
            break
    
    if config_file is None:
        print(f"Warning: No config file found for run identifier '{run_identifier}'. Prior plotting will be disabled.")
        print(f"Searched in: {potential_config_files}")
        return None
...
def auto_detect_results_file():
    """Auto-detect the results file in the current directory.
    
    Returns:
        tuple: (results_file_path, run_identifier) where run_identifier is extracted from the file path
    """
    current_dir = "."
    ...
    for root, dirs, files in os.walk(current_dir):
        for file in files:
            if file.endswith('_results.nc'):
                full_path = os.path.join(root, file)
                results_files.append(full_path)
    
    if len(results_files) == 0:
        print("Error: No results files (*_results.nc) found in current directory tree!")
        sys.exit(1)
    elif len(results_files) > 1:
        print(f"Error: Multiple results files found: {results_files}")
        print("Please ensure only one results file is present in the directory tree.")
        sys.exit(1)
    
    results_file = results_files[0]
    
    # Extract run identifier from file path
    # Expected pattern: numpyro_test_{run_index}/numpyro_test_{run_index}_results.nc
    # or more general: */{some_identifier}_results.nc
    filename = os.path.basename(results_file)
    if filename.endswith('_results.nc'):
        run_identifier = filename[:-11]  # Remove '_results.nc'
        # If it starts with pattern like 'numpyro_test_', extract the suffix
        if run_identifier.startswith('numpyro_test_'):
            run_identifier = run_identifier[13:]  # Remove 'numpyro_test_'
    else:
        run_identifier = "unknown"
    
    print(f"Auto-detected results file: {results_file}")
    print(f"Extracted run identifier: {run_identifier}")
    
    return results_file, run_identifier
...
# Auto-detect results file and extract run identifier
results_file, run_identifier = auto_detect_results_file()

# Load config file for prior printing and plotting
config = load_config(run_identifier)
...
fig.suptitle(f'Run {run_identifier} Parameter Posterior Distributions\n(log10_ha{log10_gamma_a_text}{efac_text}{equad_text} + {num_pulsars} Pulsars)', 
             fontsize=16, y=0.98)

# Ensure plots directory exists
plots_dir = os.path.join(os.path.dirname(results_file), "plots")
...
output_file = f"{plots_dir}/corner_plot_run_{run_identifier}_{num_pulsars}pulsars{smooth_suffix}{log10_gamma_a_suffix}{efac_suffix}{equad_suffix}{priors_suffix}{injections_suffix}.png"
