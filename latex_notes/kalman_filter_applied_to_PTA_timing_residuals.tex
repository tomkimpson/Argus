...
}\author{\small T. Kimpson} % Your name
...
\section{System definition in continuous time}\label{sec:continuous_time_system}
We have a set of $N$ pulsars. Each pulsar has $4 + M^{(n)}$ states, where $M^{(n)}$ is the number of timing model parameters for the $n$-th pulsar \footnote{These are things like corrections to RA/DEC, DM, etc. Typically $M^{(n)} \sim 10$ or so.}. The states evolve according to the following set of dynamical equations.
\begin{align}
...
The stochastic process  $\chi_p^{(n)}$ is uncorrelated between different $n$ \footnote{i.e. timing noise is uncorrelated between pulsars}. The statistics of $\chi_p^{(n)}$ are defined by
\begin{align}
	\langle \chi_p^{(n)}(t) \rangle &= 0 \ , \\
	\langle \chi_p^{(n)}(t) \chi_p^{(n')}(t') \rangle &= [\sigma_p^{(n)}]^2 \delta_{n,n'} \delta(t - t') \ .
\end{align}
\footnote{The factor $\delta_{n,n'}$ in the right-hand side ensures that the non-GW-induced spin fluctuations in distinct pulsars $n\neq n'$ are uncorrelated, as expected physically.}


The stochastic process $\chi_{\mathrm a}^{(n)}$  is correlated between pulsars - this is the usual Hellings-Downs correlation. The ensemble statistics of $\chi_{\mathrm a}^{(n)}$ are
\begin{align}
...
The states $\delta \epsilon^{(n)}_{m}$ are static, as they represent parameters. In a typical PTA analysis one marginalises over these timing model parameters. Here, we set them as static with some initial value and covariance.  \footnote{That is, we just set these with some Gaussian prior}. \newline 
...
\begin{equation}
	\delta t^{(n)} = \frac{\delta \phi^{(n)}}{f_0^{(n)}} + \mathbf{M}^{(n)} \boldsymbol{\delta \epsilon}^{(n)} - r^{(n)} \label{eq:meas}
\end{equation}
where $\mathbf{M}^{(n)}$ is the design matrix of partial derivatives of the phases with respect to the timing model parameters. It is provided by standard pulsar timing libraries such as TEMPO and is a (row)vector of dimension $M^{(n)}$. \footnote{Note the clumsy notation; $\mathbf{M}^{(n)}$ is a vector while $M^{(n)}$ is a scalar which sets the dimension of $\mathbf{M}^{(n)}$. In standard PSR timing notation $\mathbf{M}^{(n)}$ is a matrix which goes over all times; here we can just consider it to be a time-varying vector.} The quantity $\boldsymbol{\delta \epsilon}^{(n)}$ is a (column) vector, also of length $M^{(n)}$, which holds all the $\delta \epsilon^{(n)}_{m}$ terms for pulsar $n$.
...
The structure of the equations in Section \ref{sec:continuous_time_system} suggests a block organisation of the state vector as follows. \newline 

There are three blocks which are uncorrelated.
\begin{itemize}
	\item The GW block, $r^{(n)}$ and $a^{(n)}$.
	\item The spin-down block, $\delta \phi^{(n)}$ and $\delta f^{(n)}$.
	\item The timing model parameters block, $\delta \epsilon^{(n)}_{m}$.
\end{itemize}
We will note these as $\boldsymbol{X}_{\text{gw}}$, $\boldsymbol{X}_{\text{spin}}$ and $\boldsymbol{X}_{\text{tm}}$ respectively. The states stretch across all pulsars. So for example $\boldsymbol{X}_{\text{spin}}  = \left(\delta \phi^{(1)}, \delta f^{(1)}, \delta \phi^{(2)}, \delta f^{(2)}, \cdots, \delta \phi^{(N)}, \delta f^{(N)}\right)$. We summarise the $\boldsymbol{F}$ and $\boldsymbol{Q}$ matrices of each of the blocks below. Their derivation can be found in Appendix \ref{sec:derivation_of_matrices}.
...
where $I_N$ is the $N\times N$ identity matrix and $\otimes$ denotes the Kronecker product. \footnote{For all pulsars, the deterministic part of the GW is the same, governed by $\gamma_{\rm a}$. The per-pulsar dependence arises in the stochastic part / Q-matrix.}

\hrule \vspace{2mm}

\noindent \textbf{Spin-down block}
...
\noindent \textbf{Timing ephemeris block}
...
In general our matrices are large and sparse. This introduces computational challenges \footnote{Slow matrix multiplication, numerical stability and cost of matrix inverse,...}. We can leverage the block diagonal structure of $\mathbf{F}$ and $\mathbf{Q}$, and the simple form of the measurement equation, Equation \eqref{eq:meas}, we can sidestep some of these issues. \newline 

Specifically, we will now handle 3 state vectors
 $\boldsymbol{X}_{\text{gw}}  \boldsymbol{X}_{\phi/f} \boldsymbol{X}_{\delta\epsilon}$ and the 6 unique block components of the covariance matrix, separately. 
...
\end{equation}
with covariance \footnote{at a given timestep, e.g. after the update step}
\begin{equation}
	P = \begin{pmatrix}
	...
	\end{pmatrix}.
\end{equation}
The new states are straightforwardly $\boldsymbol{X}(k+1) = F \boldsymbol{X}(k)$, or
\begin{align}
...
\end{align}
The new covariances are $P_p = F\,P\,F^T + Q$, which expands to
\begin{equation}
	P_p = \begin{pmatrix}
	...
	\end{pmatrix}.
\end{equation}
Because $P$ is symmetric, we only need to track 6 blocks\footnote{3 diagonal, 3 off diagonal}
\begin{itemize}
...
At time step $k$, we obtain a single scalar measurement $y_k$ with a $1\times n_x$ matrix (row vector) $H_k$ and scalar measurement noise $R_k$. The measurement equation is
\begin{equation}
...
\begin{equation}
	...
	-
	\bigl(
	h_{\mathrm{GW}}\,x_{\mathrm{GW}}
	+
	h_{\phi/f}\,x_{\phi/f}
	+
	h_{\delta\epsilon}\,x_{\delta\epsilon}
	\bigr).
\end{equation}
To get the innovation covariance we proceed as follows. Define
\begin{equation}
	u_{\mathrm{GW}}
	=
	P_{\mathrm{GW}}\,h_{\mathrm{GW}}^\mathsf{T}
	+
	P_{\mathrm{GW},\,\phi/f}\,h_{\phi/f}^\mathsf{T}
	+
	P_{\mathrm{GW},\,\delta\epsilon}\,h_{\delta\epsilon}^\mathsf{T},
\end{equation}

\begin{equation}
	u_{\phi/f}
	=
	P_{\phi/f,\,\mathrm{GW}}\,h_{\mathrm{GW}}^\mathsf{T}
	+
	P_{\phi/f}\,h_{\phi/f}^\mathsf{T}
	+
	P_{\phi/f,\,\delta\epsilon}\,h_{\delta\epsilon}^\mathsf{T},
\end{equation}

\begin{equation}
	u_{\delta\epsilon}
	=
	P_{\delta\epsilon,\,\mathrm{GW}}\,h_{\mathrm{GW}}^\mathsf{T}
	+
	P_{\delta\epsilon,\,\phi/f}\,h_{\phi/f}^\mathsf{T}
	+
	P_{\delta\epsilon}\,h_{\delta\epsilon}^\mathsf{T}.
\end{equation}
and
\begin{equation}
	u
	...
	\end{pmatrix}
	=
	P_{k\mid k-1}\,H_k^\mathsf{T}.
\end{equation}
\footnote{$u_{\mathrm{GW}}$ (e.g.) is shape $(2N,1)$}. The innovation covariance is a scalar:
\begin{equation}
	S_k
	=
	H_k\,P_{k\mid k-1}\,H_k^\mathsf{T} 
	+ 
	R_k
	=
	\bigl(h_{\mathrm{GW}},\,h_{\phi/f},\,h_{\delta\epsilon}\bigr)
	\begin{pmatrix}
		u_{\mathrm{GW}}\\
		u_{\phi/f}\\
		u_{\delta\epsilon}
...
\begin{equation}
	S_k 
	=
	h_{\mathrm{GW}}\,u_{\mathrm{GW}}
	+
	h_{\phi/f}\,u_{\phi/f}
	+
	h_{\delta\epsilon}\,u_{\delta\epsilon}
	+
	R_k.
\end{equation}
\footnote{Note that when coding this up one can take advantage of the sparsity of $H$ and just grab the corresponding entry of $u$}. 
Define $\alpha = 1/S_k$. Then
\begin{equation}
	K_k 
	= 
	\alpha\,u
	\quad\Longrightarrow\quad
	K_{\mathrm{GW}} = \alpha\,u_{\mathrm{GW}},~
	K_{\phi/f} = \alpha\,u_{\phi/f},~
	K_{\delta\epsilon} = \alpha\,u_{\delta\epsilon}.
\end{equation}
Hence each block of the state is updated via
\begin{equation}
	x_{\mathrm{GW}}^+
	=
	x_{\mathrm{GW}}
	...
	K_{\delta\epsilon}\;\nu_k.
\end{equation}	
\footnote{Look out for extra savings to be made here. $u$ is sparse, so only the pulsar-relevant component of x gets updated. }
The covariance is updated as 
\begin{equation}
P_{k\mid k}
=
(I - K_k\,H_k)\,P_{k\mid k-1}.
\end{equation}
or equivalently.
\begin{equation}
	P_{k\mid k}
	...
	\alpha
	\,
	\bigl(u\,u^\mathsf{T}\bigr).
\end{equation}
That is to say, in block form, each block is updated by subtracting the appropriate outer products. For example,
\[
P_{\mathrm{GW}}^+
=
P_{\mathrm{GW}}
-
\alpha\,u_{\mathrm{GW}}\,u_{\mathrm{GW}}^\mathsf{T},
\quad
P_{\mathrm{GW},\,\phi/f}^+
=
P_{\mathrm{GW},\,\phi/f}
-
\alpha\,u_{\mathrm{GW}}\,u_{\phi/f}^\mathsf{T},
\]
and so on for all 6 blocks \(\{P_{\mathrm{GW}},P_{\mathrm{GW},\phi/f},\ldots\}\).
...
\section{Derivation of matrices}\label{sec:derivation_of_matrices}
