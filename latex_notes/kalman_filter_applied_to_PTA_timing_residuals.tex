...
\end{align}
where $\chi_{i}$ denotes a white noise stochastic process and $\gamma_i$ are damping constants. The processes  $\chi_p^{(n)}$ and  $\chi_\epsilon^{(n)}$ are uncorrelated between different $n$. Their statistics are defined by
...
\end{align}
\footnote{I am not sure if it makes sense for the timing model parameter errors to be treated in this way; perhaps there is some correlation between the parameters for a single pulsar . For now we just assume that they are all uncorrelated for simplicity.} \newline 
