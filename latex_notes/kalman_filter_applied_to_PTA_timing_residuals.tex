...
\section{System definition: continuous ODEs}
...
The previous section defines the continuous-time system for a single pulsar. We now discretise it in time for multiple pulsars.

\subsection{Block organisation}

It is tempting to define the state vector as
\begin{equation}
\boldsymbol{X} = \left(\boldsymbol{X}^{(1)}, \boldsymbol{X}^{(2)},\cdots, \boldsymbol{X}^{(N)} \right),
\end{equation}
where $\boldsymbol{X}^{(n)} = \left(\delta \phi^{(n)}, \delta f^{(n)}, r^{(n)}, a^{(n)}, \delta \epsilon^{(n)}_{m} \right)$ is the state vector for pulsar $n$. However, it will prove more useful to define the state vector  in terms of a series of blocks
\begin{equation}
\boldsymbol{X} = \left(\boldsymbol{X}_{\text{spin}}, \boldsymbol{X}_{\text{gw}}, \boldsymbol{X}_{\text{tm}} \right),
\end{equation}
where $\boldsymbol{X}_{\text{spin}} = \left(\boldsymbol{X}^{(1)}_{\text{spin}}, \boldsymbol{X}^{(2)}_{\text{spin}},\cdots, \boldsymbol{X}^{(N)}_{\text{spin}} \right)$, $\boldsymbol{X}_{\text{gw}} = \left(\boldsymbol{X}^{(1)}_{\text{gw}}, \boldsymbol{X}^{(2)}_{\text{gw}},\cdots, \boldsymbol{X}^{(N)}_{\text{gw}} \right)$, and $\boldsymbol{X}_{\text{tm}} = \left(\boldsymbol{X}^{(1)}_{\text{tm}}, \boldsymbol{X}^{(2)}_{\text{tm}},\cdots, \boldsymbol{X}^{(N)}_{\text{tm}} \right)$.
...
\section{Optimised Kalman filter implementation}

The block structure of the state and covariance allows for a more efficient implementation of the Kalman filter.

\noindent Lets express the state vector as 
\begin{equation}
x = \begin{pmatrix} x_{GW} \\[1mm] x_{\phi/f} \\[1mm] x_{\delta\epsilon} \end{pmatrix},
\end{equation}

The F-matrix and Q-matrix are block diagonal:
\begin{equation}
	F = \begin{pmatrix}
	F_{GW} & 0 & 0\\[1mm]
	0 & F_{\phi/f} & 0\\[1mm]
...
\begin{equation}
y=\frac{1}{f_0^{(n)}}\,\delta\phi+\mathbf{M}^{(n)}\,\boldsymbol{\delta\epsilon}-r,
\end{equation}
...
with
\begin{equation}
H_{GW} = \text{(zero except a }-1\text{ at the entry for } r\text{)},
\end{equation}
\begin{equation}
H_{\phi/f} = \text{(zero except } 1/f_0^{(n)} \text{ at the entry for } \delta\phi\text{)},
\end{equation}
\begin{equation}
H_{\delta\epsilon} = \text{(zero except the block for pulsar \(n\) equals } \mathbf{M}^{(n)}\text{)}.
\end{equation}
...
\begin{equation}
S = P^-_{rr} + \frac{1}{f_0^{(n)2}}\,P^-_{\delta\phi\,\delta\phi} + \mathbf{M}^{(n)}\,P^-_{\epsilon\,\epsilon}\,\mathbf{M}^{(n)T} - \frac{2}{f_0^{(n)}}\,P^-_{r,\delta\phi} - 2\,\mathbf{M}^{(n)}\,P^-_{r,\epsilon} + \frac{2}{f_0^{(n)}}\,\mathbf{M}^{(n)}\,P^-_{\delta\phi,\epsilon} + R.
\end{equation}
...
\begin{equation}
K=\begin{pmatrix}
K_{GW}\\[1mm]K_{\phi/f}\\[1mm]K_{\delta\epsilon}
\end{pmatrix}=
\begin{pmatrix}
P^-_{GW}H_{GW}^T\\[1mm]P^-_{\phi/f}H_{\phi/f}^T\\[1mm]P^-_{\delta\epsilon}H_{\delta\epsilon}^T
\end{pmatrix}\,S^{-1}.
\end{equation}
Because \(H_{GW}\) has only one nonzero entry (namely, \(-1\) at the \(r\) position), we have
\begin{equation}
K_{GW} = -\frac{P^-_{GW}(:,r)}{S},
\end{equation}
and similarly,
\begin{equation}
K_{\phi/f} = \frac{P^-_{\phi/f}(:,\delta\phi)}{f_0^{(n)}\,S},\qquad
K_{\delta\epsilon} = \frac{P^-_{\delta\epsilon}(:,\epsilon)\,\mathbf{M}^{(n)T}}{S}.
\end{equation}

The state update is
\begin{equation}
x^+ = x^- + K\Bigl(y-H\,x^-\Bigr).
\end{equation}
...
\begin{equation}
P^+_{rr} = P^-_{rr} - \frac{\left(P^-_{rr}\right)^2}{S},
\end{equation}
and similarly for the other blocks and the cross–terms.



\subsection{Update step again}

Assume that after the predict step we have a state
\begin{equation}
x_{k\mid k-1}
=
\begin{pmatrix}
...
x_{\phi/f}\\[3pt]
x_{\delta\epsilon}
\end{pmatrix}_{\!k\mid k-1}
\quad\in \mathbb{R}^{n},
\end{equation}
and a block-partitioned covariance
\begin{equation}
P_{k\mid k-1}
=
\begin{pmatrix}
P_{\mathrm{GW}} & P_{\mathrm{GW},\,\phi/f} & P_{\mathrm{GW},\,\delta\epsilon}\\[5pt]
P_{\phi/f,\,\mathrm{GW}} & P_{\phi/f} & P_{\phi/f,\,\delta\epsilon}\\[5pt]
P_{\delta\epsilon,\,\mathrm{GW}} & P_{\delta\epsilon,\,\phi/f} & P_{\delta\epsilon}
\end{pmatrix}_{k\mid k-1}.
\end{equation}
Here, each block corresponds to a portion of the state:
\begin{equation}
\textstyle
x_{\mathrm{GW}}\in \mathbb{R}^{2N},\quad
x_{\phi/f}\in \mathbb{R}^{2N},\quad
x_{\delta\epsilon}\in \mathbb{R}^{\sum M^{(n)}}.
\end{equation}
At time step $k$, we obtain a single scalar measurement $y_k$ with a $1\times n$ matrix (row vector) $H_k$ and scalar measurement noise $R_k$. The measurement equation is
\begin{equation}
y_k = H_k\,x_{k\mid k-1} + \text{(noise)}.
\end{equation}


Often, $H_k$ is mostly zeros (e.g., if we only measure one pulsar's residual). We can exploit that structure and avoid constructing large matrices. Let us partition $H_k$ into three sub-rows,
\begin{equation}
H_k 
=
\bigl[
...
\bigr].
\end{equation}
Below, we detail how to compute the necessary products and update each block of the covariance.

\paragraph{3.1 Compute the Vector \(\boldsymbol{u}\).}
...
Stack them into one column vector
\begin{equation}
u
=
\begin{pmatrix}
...
\paragraph{3.2 Compute \(S_k\) (Scalar).}
...
\paragraph{3.4 Rank-1 Covariance Update.}
...
\]
In block form, each block is updated by subtracting the appropriate outer products. For example,
\[
P_{\mathrm{GW}}^+
=
P_{\mathrm{GW}}
...
\noindent
\textbf{Remark:} 
Even though only one pulsar may be observed at time $k$, if the GW block couples different pulsars, the rank-1 subtraction automatically updates those cross-covariances as well.
