...
def setup_output_directory(config, use_gw, timestamp=None, config_path=None):
    ...
    base_output_dir = os.path.join(
        project_root, "workflows", workflow_name, "outputs", base_dir
    )
...
def setup_single_logger(config, output_dir=None, enable_file_logging=None):
    ...
    if enable_file_logging is None:
        enable_file_logging = config.getboolean(
            "Logging", "enable_file_logging", fallback=True
        )
    ...
    if enable_file_logging:
        if output_dir is None:
            raise ValueError(
                "output_dir must be provided when enable_file_logging=True"
            )

        # Create timestamp for log file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_file = os.path.join(
            output_dir, "logfiles", f"inference_output_{timestamp}.txt"
        )
