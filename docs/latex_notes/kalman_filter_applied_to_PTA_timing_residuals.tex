...
\begin{align}
	...
	\frac{d}{dt} r^{(n)} &= a^{(n)}, \\
	\frac{d}{dt} a^{(n)} &= -\gamma_a a^{(n)} + \chi_{\mathrm a}^{(n)}(t;\sigma_a^{(n)}), \\
	\frac{d}{dt} \delta \epsilon^{(n)}_{m} &= \chi_\epsilon^{(n)}(t;\sigma_\epsilon^{(n)}) \quad \forall m \in [1, M^{(n)}].
\end{align}
where $\chi_{\mathrm a}$ denotes a white noise stochastic process. The processes  $\chi_p^{(n)}$ and  $\chi_\epsilon^{(n)}$ are uncorrelated between different $n$. Their statistics are defined by
...
dfdfdf 
The correlation between pulsars manifests due to the GW, i.e. the $a^{(n)}$ state. The statistics here are term whilst the ensemble statistics of $\chi_{\mathrm a}^{(n)}$ are
\begin{align}
...
\begin{equation}
	\delta t^{(n)} = \frac{\delta \phi^{(n)}}{f_0^{(n)}} + \mathbf{M}^{(n)} \mathbf{\delta \epsilon}^{(n)} - r^{(n)}
\end{equation}
