...
# Helper script to create a configuration file for full parameter inference using the Argus CLI

echo "=== Creating Argus Full Inference Configuration File ==="
echo
...
if [ $? -eq 0 ]; then
    ...
    echo "  [Data] - Set data_path (avoid noise_params_path for full inference)"
    echo "  [Inference] - Set nsamples, nwarmup, nchains"
    echo "  [PriorModel] - Set parameter ranges (comment out fixed parameter paths for full inference)"
