...
- [x] **T3.5 — Scale to the full NG15 array.** 🛑 *machinery validated + first-look amplitude;
  the converged full-joint run is **OFF THE TABLE**: the ~142-D joint posterior was ruled
  **unsamplable** by NUTS (2026-07-18; three tuning levers — diagonal mass, dense GW block at
  `target_accept` 0.95 and 0.99 — triangulate a divergence-vs-mixing wall; see issue #109 and
  the `log.md` entry). Superseded by the two-stage-noise roadmap in
  `docs/design/2026-07-19-real-data-roadmap.md` (issue #111); the filter machinery, full-array
  data prep, and HD-diagonal fix below all carry forward.*
