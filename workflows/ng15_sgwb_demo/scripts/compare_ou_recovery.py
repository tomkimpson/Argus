...
def _tspan_yr_from_feathers(data_path):
    ...
    if not files:
        raise SystemExit(f"no feathers in {data_path} (needed for T_span in --published mode)")
...
def run_published(args):
    ...
    for label, f in pivots.items():
        ...
        for key, ref in PUBLISHED_NG15.items():
            s_ref = float(powerlaw_psd(np.array([f]), ref["log10_A_gw"], ref["gamma"])[0])
            ...
            print(f"    published log10 PSD = {l_ref:+.3f}  |  bias {bias_psd_dex:+.3f} dex PSD "
    ...
    comparison["gamma_a_rail_check"] = {
        ...
        "railed": bool((ga_s[0] - gmin < args.rail_tol) or (gmax - ga_s[0] < args.rail_tol)
                       or frac_near_lo > 0.3 or frac_near_hi > 0.3),
    }
    ...
    ha_railed = bool((ha_s[0] - hmin < args.rail_tol) or (hmax - ha_s[0] < args.rail_tol)
                     or frac_ha_near_lo > 0.3 or frac_ha_near_hi > 0.3)
    ...
    print("-" * 78)
    print(f"log10_ha off prior edges (prior [{hmin}, {hmax}]): railed={ha_railed} "
          f"(median {ha_s[0]:+.2f}, dist low {ha_s[0]-hmin:.2f} / high {hmax-ha_s[0]:.2f} dex)")
    print(f"gamma_a rail check (prior [{gmin}, {gmax}]): "
    ...
    f_grid = np.logspace(np.log10(1.0 / (T_yr * SEC_PER_YEAR)), np.log10(2.0 * F_YR), 200)
    ...
    ax.fill_between(f_grid, ou_lo, ou_hi, color="C0", alpha=0.25, label="recovered OU (68%)")
    for color, (key, ref) in zip(("C3", "C2"), PUBLISHED_NG15.items()):
        ax.plot(f_grid, powerlaw_psd(f_grid, ref["log10_A_gw"], ref["gamma"]),
                color=color, lw=2, ls="--", label=ref["label"])
    for label, f in pivots.items():
        ax.axvline(f, color="0.5", ls=":", lw=1)
        ax.text(f, 1, label, rotation=90, va="bottom", ha="right", fontsize=8, color="0.4",
                transform=ax.get_xaxis_transform())
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("frequency [Hz]")
    ax.set_ylabel("residual PSD [s$^3$]")
    ax.set_title(f"T3.3 spectral overlay: OU recovery vs published NG15 SGWB\n"
    ...
    comparison["verdict"] = {
        ...
        "vs_fixed_gamma_13_3": {
            ...
            "overlaps": primary["vs_fixed_gamma_13_3"]["recovered_hdi94_overlaps_published"],
        },
        "vs_free_gamma": {
    ...
    for key in ("vs_fixed_gamma_13_3", "vs_free_gamma"):
        print(f"VERDICT @ f=1/(5yr) {key}: band-amp bias "
...
def main():
    ...
    ap.add_argument("--data-path", default=os.path.join(ROOT, "data", "aligned"),
                    help="aligned feather dir, for computing T_span in --published mode")
    ap.add_argument("--ha-prior", nargs=2, type=float, default=[-17.0, -11.0],
                    metavar=("MIN", "MAX"),
