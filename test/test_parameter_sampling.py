...
class TestSamplePulsarNoiseParameters:
    ...
    def test_with_hierarchical_modeling(self, mock_log_ratio, mock_hierarchical):
        ...
        result = parameter_sampling.sample_pulsar_noise_parameters(prior_specs, n_pulsars)
    ...
    def test_efac_distribution(self, mock_deterministic, mock_sample):
        ...
        prior_specs = {
            "efac_spec": tfpd.Uniform(
                jnp.full(n_pulsars, 0.5),
                jnp.full(n_pulsars, 2.0)
            ),
            "equad_spec": jnp.full(n_pulsars, 1e-7)
        }

        mock_sample.return_value = jnp.ones(n_pulsars)
        mock_deterministic.side_effect = [jnp.ones(n_pulsars), jnp.full(n_pulsars, 1e-7)]
        ...
        prior_specs = {
            ...
            "equad_spec": {
                ...
                "log10_equad_spec": tfpd.Uniform(
                    jnp.full(n_pulsars, -8.0),
                    jnp.full(n_pulsars, -6.0)
                )
            }
        }
...
class TestCountFreeParameters:
    ...
    def test_count_with_reparameterized_ha(self):
        ...
        prior_specs = {
            ...
            "equad_spec": {"use_log10": True, "log10_equad_spec": tfpd.Uniform(jnp.zeros(2), jnp.ones(2))}
        }
    ...
    def test_count_all_free(self):
        ...
        prior_specs = {
            ...
            "equad_spec": {"use_log10": True, "log10_equad_spec": tfpd.Uniform(jnp.zeros(3), jnp.ones(3))}
        }
